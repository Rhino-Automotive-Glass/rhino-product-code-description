...
## Database Setup: Baseline Schema and Pending Migrations

`npm run e2e:db` (`scripts/e2e-db.sh`) builds the throwaway local database the
tests run against. It never touches production, and this repo never runs
`supabase db push` — production is shared with other Rhino apps and has drifted
from `supabase/migrations/`, which is why `supabase/config.toml` keeps
`db.migrations.enabled = false`.

### Load order

1. Drop and recreate the local `public` and `private` schemas.
2. Load `supabase/e2e/schema.sql` — the **baseline**.
3. Apply every pending migration (`scripts/e2e-apply-migrations.sh`).
4. Load `supabase/e2e/seed.sql` — local test data.
5. Run `supabase/diagnostics/security-invariants.sql`.

Any failure stops the run immediately (`psql -X -v ON_ERROR_STOP=1`).

### schema.sql is a baseline, not the target state

`supabase/e2e/schema.sql` is generated by `npm run e2e:schema` from a real
production backup. It is a snapshot of production **as of that backup** — it
already contains every migration that has run there, and it does not contain
migrations that have not. Treating it as the final state is what used to let a
broken migration pass CI: the tests validated a hand-maintained snapshot rather
than the SQL that was about to run against production.

### The manifest

`supabase/e2e/migrations-in-schema.txt` lists, one bare filename per line, the
migrations already baked into the baseline. `#` comments and blank lines are
allowed. The list is explicit rather than derived from timestamps because
`supabase/migrations/` mixes legacy names (`001_*.sql`, `add_verified_column.sql`)
with timestamped ones, so sorting cannot tell old from new.

Everything in `supabase/migrations/*.sql` that the manifest does not list is
**pending** and gets applied, in byte-wise filename order. The runner fails
(without applying anything) if a manifest entry has no matching file, is listed
twice, contains a path separator or `..`, or does not end in `.sql`.

### Adding a migration

Add the `.sql` file to `supabase/migrations/` and **do not** add it to the
manifest. That is what makes `npm run e2e:db` — and therefore every pull
request — execute it on top of the baseline. If it fails, CI fails.

### Regenerating the baseline

Do these two things in the **same commit**, or the migration is either replayed
or silently skipped:

1. `npm run db:backup` on a production database that already has the migration,
   then `npm run e2e:schema` to regenerate `supabase/e2e/schema.sql`.
2. Add the now-included migration filenames to
   `supabase/e2e/migrations-in-schema.txt`.

Then run `npm run e2e:db` and confirm the runner reports the migration as no
longer pending.
...
| Command | Checks |
|---------|--------|
| `bash supabase/tests/migration_runner.sh` | Migration selection and execution rules (throwaway database + temp fixture directories) |
| `psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f supabase/tests/pending_migrations_applied.sql` | The pending catalog-sync migration really ran: `private.ensure_catalog_entry(uuid)` and `products_product_code_id_unique_idx` exist |
| `bash supabase/tests/catalog_sync_concurrency.sh` | Concurrent inserts and overlapping backfills |
| `bash supabase/tests/catalog_sync_migration_repeat.sh` | The backfill migration is repeatable |
...
- Loads the baseline schema and applies every pending migration (`npm run e2e:db`),
  so a broken migration fails the pull request instead of the deploy
- Runs the migration-runner regression suite and asserts the pending migrations
  really ran
...
| `npm run e2e:db` | Rebuild the local test database: baseline schema + pending migrations + seed |
| `npm run e2e:schema` | Regenerate the baseline `supabase/e2e/schema.sql` from a production backup |
